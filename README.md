...
# 项目概述
1. 简介：HavenCamp 是一个前后端分离的即时通讯项目，具备后台管理、单聊群聊、联系人管理、多种消息（文本 / 文件 / 视频）处理、离线消息处理以及音视频通话等功能，旨在打造类似微信的聊天体验。
2. 技术栈：
...
# 自动创建数据库
echo "Creating database 'haven_camp_server'..."
sudo mysql -u root -p <<EOF
CREATE DATABASE haven_camp_server;
EOF
...
# 重新安装项目依赖
cd ~/project/HavenCamp/web/chat-server
...
rm -rf /var/www/html/* 
rm -rf /root/project/HavenCamp/web/chat-server/dist
yarn build
sudo cp -r /root/project/HavenCamp/web/chat-server/dist/* /var/www/html # 改成自己的项目路径
sudo chmod -R 755 /var/www/html
sudo chown -R www-data:www-data /var/www/html

cd ~/project/HavenCamp
...
# 将后端打包部署
cd ~/project/HavenCamp/cmd/haven_camp_server # 里面是main.go
go build -o haven_camp_backend main.go
sudo cp haven_camp_backend /usr/local/bin/

sudo nano /etc/systemd/system/haven_camp_backend.service
# 配置以下内容
# [Unit]
# Description=haven camp service
# After=network.target

# [Service]
# User=haven_camp  # 替换为你的用户名
# Group=haven_camp  # 替换为你的用户名
# WorkingDirectory=/root/project/HavenCamp/cmd/haven_camp_server  # 替换为你的项目路径
# ExecStart=/usr/local/bin/haven_camp_backend  # 替换为你的可执行文件路径
# Restart=on-failure
# RestartSec=5
...
sudo systemctl start haven_camp_backend
sudo systemctl enable haven_camp_backend
